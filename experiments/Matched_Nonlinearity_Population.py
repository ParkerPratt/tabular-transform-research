...
EXCEL_PATH = OUTPUT_DIR / "population_normalized_matched_nonlinearity_transform_mismatch_n500_p50_a10.xlsx"
...
def make_paired_representations(n, means, variances, target_corr, seed, transform_name, strength):
    ...
    base_representation = Z_raw * np.sqrt(variances) + means

    a = strength
    if transform_name == "cubic":
        mu, var = 0.0, 1 + 6*a + 15*a**2
    elif transform_name == "quintic":
        mu, var = 0.0, 1 + 30*a + 945*a**2
    elif transform_name == "quad":
        mu, var = a, 1 + 4*a**2 + (5/3)*a**4
    elif transform_name == "exp":
        if np.isclose(a, 0.0):
            mu, var = 0.0, 1.0
        else:
            mu = np.expm1(a**2 / 2) / a
            var = np.exp(a**2) * np.expm1(a**2) / a**2

    transformed_raw = TRANSFORMS[transform_name](Z_raw, a)
    transformed_representation = (
        (transformed_raw - mu) / np.sqrt(var)
    ) * np.sqrt(variances) + means
