import json
from pathlib import Path

import numpy as np
import pandas as pd
from numpy.linalg import cholesky, eig
from scipy.stats import loguniform
from sklearn.metrics import mean_squared_error, r2_score
from sklearn.model_selection import KFold, RandomizedSearchCV
from sklearn.neural_network import MLPRegressor
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler


GLOBAL_SEED = 42
np.random.seed(GLOBAL_SEED)

OUTPUT_DIR = Path("../results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
EXCEL_PATH = OUTPUT_DIR / "four_condition_diagnostics_modern.xlsx"

P = 50
N = 500
ACTIVE = 10
N_REPS = 25
MASTER_SEED = 1337

TARGET_SIGNAL_SCALE = 1.0
TARGET_NOISE_SD = 1.0
TARGET_INTERCEPT = 0.0
TEST_FRACTION = 0.2

LEVELS_TO_RUN = [0.30]

ACTIVE_TRANSFORM_COUNTS = list(range(1, ACTIVE + 1))

TRANSFORM_CONFIGS = [
    {
        "name": f"active_{k}",
        "n_active_transformed": k,
        "n_inactive_transformed": 0,
    }
    for k in ACTIVE_TRANSFORM_COUNTS
]
TRANSFORM_CONFIGS += [
    {"name": "inactive_all", "n_active_transformed": 0, "n_inactive_transformed": P - ACTIVE},
    {"name": "all_features", "n_active_transformed": ACTIVE, "n_inactive_transformed": P - ACTIVE},
]

CONDITIONS = [
    ("base", "base"),
    ("base", "transformed"),
    ("transformed", "base"),
    ("transformed", "transformed"),
]

def transform_identity(z, a):
    return np.asarray(z, dtype=float)


def transform_cubic(z, a):
    """T(z) = z + a z^3. Strictly increasing for a >= 0."""
    z = np.asarray(z, dtype=float)
    return z + a * z**3


def transform_quintic(z, a):
    """T(z) = z + a z^5. Strictly increasing for a >= 0."""
    z = np.asarray(z, dtype=float)
    return z + a * z**5


def transform_exp(z, a):
    """T(z) = expm1(a z) / a, with identity at a = 0. Strictly increasing."""
    z = np.asarray(z, dtype=float)
    if np.isclose(a, 0.0):
        return z
    return np.expm1(a * z) / a


def transform_quad_cubic(z, a):
    """T(z) = z + a z^2 + (a^2 / 3) z^3; derivative is nonnegative."""
    z = np.asarray(z, dtype=float)
    return z + a * z**2 + (a**2 / 3.0) * z**3


TRANSFORMS = {
    "cubic": transform_cubic,
    "quintic": transform_quintic,
    "exp": transform_exp,
    "quad": transform_quad_cubic,
}

MATCHED_STRENGTHS = {
    "cubic": {
        0.05: 0.1302579,
        0.10: 0.22996560,
        0.15: 0.3532381,
        0.20: 0.5265986,
        0.30: 1.3483315,
    },
    "quintic": {
        0.05: 0.0098076,
        0.10: 0.0152675,
        0.15: 0.0204604,
        0.20: 0.0258628,
        0.30: 0.038476,
    },
    "exp": {
        0.05: 0.3189425,
        0.10: 0.4551335,
        0.15: 0.5627497,
        0.20: 0.6563857,
        0.30: 0.8218707,
    },
    "quad": {
        0.05: 0.1659280,
        0.10: 0.2476384,
        0.15: 0.3223790,
        0.20: 0.3994894,
        0.30: 0.5912237,
    },
}


def make_target_continuous(signal, noise, signal_scale=1.0, intercept=0.0, w=None):
    """Generate y from a supplied signal representation using fixed noise."""
    if w is None:
        w = np.ones(signal.shape[1])
    w = np.asarray(w, dtype=float)
    return intercept + signal_scale * (signal @ w) + noise


def standardize_and_scale(A, means, variances):
    """Give each sample marginal exactly the requested sample mean/variance."""
    A = np.asarray(A, dtype=float)
    means = np.asarray(means, dtype=float)
    variances = np.asarray(variances, dtype=float)

    sd = A.std(axis=0, ddof=0)
    if np.any(sd == 0):
        raise ValueError("A generated feature has zero sample variance.")

    A_std = (A - A.mean(axis=0)) / sd
    return A_std * np.sqrt(variances) + means


def _nearest_spd(A, eps=1e-9):
    A = (A + A.T) / 2.0
    vals, vecs = eig(A)
    vals = np.real(vals)
    vecs = np.real(vecs)
    vals[vals < eps] = eps
    return vecs @ np.diag(vals) @ vecs.T


def _cholesky_spd(A):
    try:
        return cholesky(A)
    except np.linalg.LinAlgError:
        return cholesky(_nearest_spd(A))


def make_transform_mask(p, n_active, n_active_transformed, n_inactive_transformed):
    """Construct a mask over relevant and irrelevant predictors separately."""
    mask = np.zeros(p, dtype=bool)

    n_active_transformed = min(int(n_active_transformed), n_active)
    n_inactive_transformed = min(int(n_inactive_transformed), p - n_active)

    mask[:n_active_transformed] = True
    mask[n_active : n_active + n_inactive_transformed] = True
    return mask


def make_paired_representations(
    n,
    means,
    variances,
    target_corr,
    seed,
    transform_name,
    strength,
    transform_mask,
):
    """
    Generate base Gaussian and transformed representations from identical Z draws.

    Only columns selected by transform_mask are transformed. This lets the same
    modern mismatch DGP test active-only, inactive-only, and all-feature controls.
    """
    means = np.asarray(means, dtype=float)
    variances = np.asarray(variances, dtype=float)
    target_corr = np.asarray(target_corr, dtype=float)
    transform_mask = np.asarray(transform_mask, dtype=bool)

    p = len(means)
    if means.shape != variances.shape or target_corr.shape != (p, p):
        raise ValueError("Incompatible data-generation parameter dimensions.")
    if transform_mask.shape != (p,):
        raise ValueError("transform_mask must have shape (p,).")

    rng = np.random.default_rng(seed)
    L = _cholesky_spd(target_corr)
    Z_raw = rng.standard_normal(size=(n, p)) @ L.T

    base_representation = standardize_and_scale(Z_raw, means, variances)

    transformed_raw = Z_raw.copy()
    transform_fn = TRANSFORMS[transform_name]
    if np.any(transform_mask):
        transformed_raw[:, transform_mask] = transform_fn(
            Z_raw[:, transform_mask], strength
        )

    transformed_representation = standardize_and_scale(
        transformed_raw, means, variances
    )

    return base_representation, transformed_representation


def make_weights(p, n_active):
    n_active = min(n_active, p)
    return np.r_[np.ones(n_active), np.zeros(p - n_active)]


# =============================================================================
# ANN fitting: keep the same tuning structure as the supplied diagnostic script
# =============================================================================

CV = KFold(n_splits=5, shuffle=True, random_state=2025)

PARAM_DISTRIBUTIONS_ANN = {
    "mlp__hidden_layer_sizes": [(32,), (64,), (32, 16), (64, 32)],
    "mlp__alpha": loguniform(1e-5, 1e-2),
    "mlp__learning_rate_init": loguniform(1e-4, 1e-2),
    "mlp__batch_size": [32, 64, 128],
}

N_ITER_ANN = 10


def fit_ann(X_train, X_test, y_train, y_test, seed):
    pipeline = Pipeline(
        [
            ("scaler", StandardScaler()),
            (
                "mlp",
                MLPRegressor(
                    random_state=seed,
                    early_stopping=True,
                    n_iter_no_change=15,
                    max_iter=500,
                ),
            ),
        ]
    )

    search = RandomizedSearchCV(
        estimator=pipeline,
        param_distributions=PARAM_DISTRIBUTIONS_ANN,
        n_iter=N_ITER_ANN,
        scoring="neg_mean_squared_error",
        cv=CV,
        n_jobs=-1,
        random_state=seed + 99,
        refit=True,
    )
    search.fit(X_train, y_train)

    best_pipeline = search.best_estimator_
    best_mlp = best_pipeline.named_steps["mlp"]

    y_pred_train = best_pipeline.predict(X_train)
    y_pred_test = best_pipeline.predict(X_test)
    mse_train = mean_squared_error(y_train, y_pred_train)
    mse_test = mean_squared_error(y_test, y_pred_test)

    validation_scores = getattr(best_mlp, "validation_scores_", None)
    if validation_scores is None:
        validation_scores = []

    n_network_parameters = int(
        sum(w.size for w in best_mlp.coefs_)
        + sum(b.size for b in best_mlp.intercepts_)
    )

    return {
        "mse_train": mse_train,
        "rmse_train": np.sqrt(mse_train),
        "r2_train": r2_score(y_train, y_pred_train),
        "mse_test": mse_test,
        "rmse_test": np.sqrt(mse_test),
        "r2_test": r2_score(y_test, y_pred_test),
        "generalization_gap_mse": mse_test - mse_train,
        "best_params": search.best_params_,
        "cv_best_mse": -search.best_score_,
        "n_iter_fitted": int(best_mlp.n_iter_),
        "loss_curve": [float(v) for v in best_mlp.loss_curve_],
        "validation_curve": [float(v) for v in validation_scores],
        "best_validation_score": float(
            getattr(best_mlp, "best_validation_score_", np.nan)
        ),
        "n_network_parameters": n_network_parameters,
    }


MODEL_FITTERS = {"ann": fit_ann}

KEY_COLUMNS = [
    "n",
    "p",
    "n_active",
    "transform",
    "mismatch_level",
    "strength",
    "transform_scenario",
    "n_active_transformed",
    "n_inactive_transformed",
    "rep",
    "observed_representation",
    "signal_representation",
    "model",
]

RESULT_COLUMNS = KEY_COLUMNS + [
    "r2_train",
    "mse_train",
    "rmse_train",
    "r2_test",
    "mse_test",
    "rmse_test",
    "generalization_gap_mse",
    "cv_best_mse",
    "n_iter_fitted",
    "best_validation_score",
    "n_network_parameters",
    "loss_curve",
    "validation_curve",
    "best_params",
    "status",
    "error_message",
]


def load_existing_results(path):
    if not path.exists():
        return pd.DataFrame(columns=RESULT_COLUMNS)

    existing = pd.read_excel(path, sheet_name="results")
    missing = set(KEY_COLUMNS) - set(existing.columns)
    if missing:
        raise ValueError(
            "Existing workbook is incompatible; missing columns: "
            f"{sorted(missing)}"
        )
    return existing


def save_results(results, path):
    results = results.sort_values(KEY_COLUMNS).reset_index(drop=True)
    successful = results.loc[results["status"] == "ok"].copy()

    if successful.empty:
        summary = pd.DataFrame()
        mismatch_summary = pd.DataFrame()
    else:
        group_cols = [
            "n",
            "p",
            "n_active",
            "transform",
            "mismatch_level",
            "strength",
            "transform_scenario",
            "n_active_transformed",
            "n_inactive_transformed",
            "observed_representation",
            "signal_representation",
            "model",
        ]

        summary = (
            successful.groupby(group_cols, as_index=False)[
                [
                    "r2_train",
                    "r2_test",
                    "mse_train",
                    "mse_test",
                    "generalization_gap_mse",
                    "n_iter_fitted",
                    "best_validation_score",
                ]
            ]
            .agg(["mean", "std"])
        )
        summary.columns = [
            "_".join(str(part) for part in col if str(part))
            if isinstance(col, tuple)
            else col
            for col in summary.columns
        ]

        # Build same-target forward/inverse mismatch penalties per replication:
        # forward = TT - BT  (T signal, compare matched vs mismatched input)
        # inverse = BB - TB  (B signal, compare matched vs mismatched input)
        pivot = successful.pivot_table(
            index=[
                "n",
                "p",
                "n_active",
                "transform",
                "mismatch_level",
                "strength",
                "transform_scenario",
                "n_active_transformed",
                "n_inactive_transformed",
                "rep",
                "model",
            ],
            columns=["observed_representation", "signal_representation"],
            values="r2_test",
            aggfunc="first",
        )

        needed = [
            ("base", "base"),
            ("base", "transformed"),
            ("transformed", "base"),
            ("transformed", "transformed"),
        ]
        if all(col in pivot.columns for col in needed):
            mismatch_summary = pivot.reset_index()
            mismatch_summary["forward_penalty"] = (
                mismatch_summary[("transformed", "transformed")]
                - mismatch_summary[("base", "transformed")]
            )
            mismatch_summary["inverse_penalty"] = (
                mismatch_summary[("base", "base")]
                - mismatch_summary[("transformed", "base")]
            )
            mismatch_summary["symmetric_penalty"] = 0.5 * (
                mismatch_summary["forward_penalty"]
                + mismatch_summary["inverse_penalty"]
            )

            # Flatten pivoted BB/BT/TB/TT column labels for Excel.
            flat_cols = []
            for col in mismatch_summary.columns:
                if isinstance(col, tuple):
                    parts = [str(x) for x in col if str(x)]
                    if len(parts) == 2 and parts[0] in {"base", "transformed"}:
                        short = {
                            ("base", "base"): "r2_BB",
                            ("base", "transformed"): "r2_BT",
                            ("transformed", "base"): "r2_TB",
                            ("transformed", "transformed"): "r2_TT",
                        }.get((parts[0], parts[1]), "_".join(parts))
                        flat_cols.append(short)
                    else:
                        flat_cols.append("_".join(parts))
                else:
                    flat_cols.append(str(col))
            mismatch_summary.columns = flat_cols
        else:
            mismatch_summary = pd.DataFrame()

    config_rows = [
        ("P", P),
        ("N", N),
        ("ACTIVE", ACTIVE),
        ("N_REPS", N_REPS),
        ("MASTER_SEED", MASTER_SEED),
        ("TARGET_NOISE_SD", TARGET_NOISE_SD),
        ("TEST_FRACTION", TEST_FRACTION),
        ("LEVELS_TO_RUN", json.dumps(LEVELS_TO_RUN)),
        ("ACTIVE_TRANSFORM_COUNTS", json.dumps(ACTIVE_TRANSFORM_COUNTS)),
        ("TRANSFORM_CONFIGS", json.dumps(TRANSFORM_CONFIGS)),
        ("N_ITER_ANN", N_ITER_ANN),
        ("PARAM_DISTRIBUTIONS_ANN", str(PARAM_DISTRIBUTIONS_ANN)),
    ]
    config_df = pd.DataFrame(config_rows, columns=["parameter", "value"])

    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        results.to_excel(writer, sheet_name="results", index=False)
        summary.to_excel(writer, sheet_name="summary", index=False)
        mismatch_summary.to_excel(writer, sheet_name="mismatch_penalties", index=False)
        config_df.to_excel(writer, sheet_name="configuration", index=False)

    print(f"[Progress] Saved {len(results):,} result rows -> {path}")


def key_tuple(record):
    return tuple(record[column] for column in KEY_COLUMNS)


# =============================================================================
# Experiment
# =============================================================================


def experiment():
    results = load_existing_results(EXCEL_PATH)
    completed_keys = {
        tuple(row)
        for row in results.loc[:, KEY_COLUMNS].itertuples(index=False, name=None)
    }
    new_rows = []

    means = np.zeros(P)
    variances = np.ones(P)
    target_corr = np.eye(P)
    weights = make_weights(P, ACTIVE)

    for transform_config in TRANSFORM_CONFIGS:
        n_active_transformed = min(
            transform_config["n_active_transformed"], ACTIVE
        )
        n_inactive_transformed = min(
            transform_config["n_inactive_transformed"], P - ACTIVE
        )
        transform_mask = make_transform_mask(
            p=P,
            n_active=ACTIVE,
            n_active_transformed=n_active_transformed,
            n_inactive_transformed=n_inactive_transformed,
        )

        for rep in range(1, N_REPS + 1):
            seed = MASTER_SEED + rep

            split_rng = np.random.default_rng(seed + 101)
            permutation = split_rng.permutation(N)
            test_size = int(np.ceil(TEST_FRACTION * N))
            test_idx = permutation[:test_size]
            train_idx = permutation[test_size:]

            noise_rng = np.random.default_rng(seed + 7)
            response_noise = noise_rng.normal(0, TARGET_NOISE_SD, size=N)

            for transform_name in TRANSFORMS:
                for mismatch_level in LEVELS_TO_RUN:
                    level = round(float(mismatch_level), 2)
                    if level not in MATCHED_STRENGTHS[transform_name]:
                        raise ValueError(
                            f"No matched strength for transform={transform_name}, "
                            f"mismatch_level={level}."
                        )
                    strength = MATCHED_STRENGTHS[transform_name][level]

                    print(
                        f"scenario={transform_config['name']} | "
                        f"rep={rep}/{N_REPS} | transform={transform_name} | "
                        f"L={level:.2f} | strength={strength:g}"
                    )

                    base_rep, transformed_rep = make_paired_representations(
                        n=N,
                        means=means,
                        variances=variances,
                        target_corr=target_corr,
                        seed=seed,
                        transform_name=transform_name,
                        strength=strength,
                        transform_mask=transform_mask,
                    )

                    representations = {
                        "base": base_rep,
                        "transformed": transformed_rep,
                    }

                    # Both targets use the same response-noise vector.
                    targets = {
                        signal_name: make_target_continuous(
                            signal=signal_matrix,
                            noise=response_noise,
                            signal_scale=TARGET_SIGNAL_SCALE,
                            intercept=TARGET_INTERCEPT,
                            w=weights,
                        )
                        for signal_name, signal_matrix in representations.items()
                    }

                    for observed_name, signal_name in CONDITIONS:
                        X = representations[observed_name]
                        y = targets[signal_name]

                        X_train, X_test = X[train_idx], X[test_idx]
                        y_train, y_test = y[train_idx], y[test_idx]

                        for model_name, fitter in MODEL_FITTERS.items():
                            key_record = {
                                "n": N,
                                "p": P,
                                "n_active": ACTIVE,
                                "transform": transform_name,
                                "mismatch_level": level,
                                "strength": strength,
                                "transform_scenario": transform_config["name"],
                                "n_active_transformed": n_active_transformed,
                                "n_inactive_transformed": n_inactive_transformed,
                                "rep": rep,
                                "observed_representation": observed_name,
                                "signal_representation": signal_name,
                                "model": model_name,
                            }

                            if key_tuple(key_record) in completed_keys:
                                continue

                            try:
                                fitted = fitter(
                                    X_train,
                                    X_test,
                                    y_train,
                                    y_test,
                                    seed,
                                )
                                row = {
                                    **key_record,
                                    "r2_train": fitted["r2_train"],
                                    "mse_train": fitted["mse_train"],
                                    "rmse_train": fitted["rmse_train"],
                                    "r2_test": fitted["r2_test"],
                                    "mse_test": fitted["mse_test"],
                                    "rmse_test": fitted["rmse_test"],
                                    "generalization_gap_mse": fitted[
                                        "generalization_gap_mse"
                                    ],
                                    "cv_best_mse": fitted["cv_best_mse"],
                                    "n_iter_fitted": fitted["n_iter_fitted"],
                                    "best_validation_score": fitted[
                                        "best_validation_score"
                                    ],
                                    "n_network_parameters": fitted[
                                        "n_network_parameters"
                                    ],
                                    "loss_curve": json.dumps(fitted["loss_curve"]),
                                    "validation_curve": json.dumps(
                                        fitted["validation_curve"]
                                    ),
                                    "best_params": json.dumps(
                                        fitted["best_params"],
                                        sort_keys=True,
                                        default=str,
                                    ),
                                    "status": "ok",
                                    "error_message": "",
                                }
                            except Exception as exc:
                                row = {
                                    **key_record,
                                    "r2_train": np.nan,
                                    "mse_train": np.nan,
                                    "rmse_train": np.nan,
                                    "r2_test": np.nan,
                                    "mse_test": np.nan,
                                    "rmse_test": np.nan,
                                    "generalization_gap_mse": np.nan,
                                    "cv_best_mse": np.nan,
                                    "n_iter_fitted": np.nan,
                                    "best_validation_score": np.nan,
                                    "n_network_parameters": np.nan,
                                    "loss_curve": "",
                                    "validation_curve": "",
                                    "best_params": "",
                                    "status": "failed",
                                    "error_message": str(exc),
                                }

                            new_rows.append(row)
                            completed_keys.add(key_tuple(key_record))

                    # Save after each transform/level block so long runs are resumable.
                    if new_rows:
                        results = pd.concat(
                            [results, pd.DataFrame(new_rows)],
                            ignore_index=True,
                        )
                        new_rows.clear()
                        save_results(results, EXCEL_PATH)

    print("Experiment complete.")


if __name__ == "__main__":
    experiment()
