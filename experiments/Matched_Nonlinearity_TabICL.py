...
TABICL_CHECKPOINT = "tabicl-regressor-v2-20260212.ckpt"
TABICL_N_ESTIMATORS = 8
TABICL_NORM_METHODS = None       # Official default preprocessing/ensemble behavior.
TABICL_OUTLIER_THRESHOLD = 4.0  # Official default.
TABICL_DEVICE = None
TABICL_VERBOSE = False

OUTPUT_DIR = Path("../results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
EXCEL_PATH = OUTPUT_DIR / f"matched_nonlinearity_tabicl_n{N}_p{P}_a{N_ACTIVE}.xlsx"
...
def fit_tabicl(X_train, X_test, y_train, y_test, seed):
    """Fit the released zero-shot TabICLv2 regressor and score the held-out set."""
    model_config = {
        "n_estimators": TABICL_N_ESTIMATORS,
        "norm_methods": TABICL_NORM_METHODS,
        "feat_shuffle_method": "latin",
        "outlier_threshold": TABICL_OUTLIER_THRESHOLD,
        "batch_size": TABICL_N_ESTIMATORS,
        "kv_cache": False,  # Only one predict() call per fit, so caching is unnecessary.
        ...
        "offload_mode": "auto",
        "random_state": seed,
        "verbose": TABICL_VERBOSE,
    }

    start = time.perf_counter()
    reg = TabICLRegressor(**model_config)
...
MODEL_FITTERS = {"tabicl_v2": fit_tabicl}
...
RESULT_COLUMNS = KEY_COLUMNS + [
    "r2_test", "mse_test", "rmse_test", "fit_seconds",
    "model_config", "status", "error_message",
]
...
def make_degradation_table(successful):
    """
    Create a convenience sheet with direction-specific R^2 degradation.

    For a fixed signal representation, compare the mismatched observed
    representation with the matched observed representation:
      signal=base:        base/base - transformed/base
      signal=transformed: transformed/transformed - base/transformed

    Positive values mean representation mismatch hurt test R^2.
    """
    if successful.empty:
        return pd.DataFrame()

    idx = ["n", "p", "n_active", "transform", "strength", "rep", "model"]
    ...
    bb = "base__base"
    bt = "base__transformed"
    tb = "transformed__base"
    tt = "transformed__transformed"

    required = {bb, bt, tb, tt}
    if not required.issubset(wide.columns):
        return pd.DataFrame()

    wide["degradation_signal_base"] = wide[bb] - wide[tb]
    wide["degradation_signal_transformed"] = wide[tt] - wide[bt]
    wide["mean_directional_degradation"] = (
        wide["degradation_signal_base"] + wide["degradation_signal_transformed"]
    ) / 2.0
    wide["direction_asymmetry"] = (
        wide["degradation_signal_transformed"] - wide["degradation_signal_base"]
    )
    return wide
...
def save_results(results, path):
    ...
    degradation = make_degradation_table(successful)
...
def run_experiment():
    ...
    total_requested_fits = (
        N_REPS * sum(len(v) for v in STRENGTHS.values()) * len(CONDITIONS) * len(MODEL_FITTERS)
    )
    print(
        f"Running TabICLv2 mismatch experiment: n={N}, p={P}, active={N_ACTIVE}, "
        f"reps={N_REPS}, requested fits={total_requested_fits:,}"
    )
    print(f"Checkpoint: {TABICL_CHECKPOINT}")
    ...
    for rep in range(1, N_REPS + 1):
        ...
        for transform_name, strengths in STRENGTHS.items():
            for strength in strengths:
                            ...
                            row = {
                                **key_record,
                                ...
                                "model_config": json.dumps(
                                    fitted["model_config"], sort_keys=True, default=str
                                ),
                                "status": "ok",
                                "error_message": "",
                            }
                ...
                # Save after each transform-strength pair so a server interruption
                if new_rows:
