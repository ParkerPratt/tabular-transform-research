...
EXCEL_PATH = OUTPUT_DIR / "inverse_transform_direction_n2000_p50_a10.xlsx"
...
def inverse_cubic(x, a):
    x = np.asarray(x, dtype=float)
    if np.isclose(a, 0.0):
        return x
    d = np.sqrt((x / (2.0 * a))**2 + 1.0 / (27.0 * a**3))
    return np.cbrt(x / (2.0 * a) + d) + np.cbrt(x / (2.0 * a) - d)


def inverse_quintic(x, a):
    x = np.asarray(x, dtype=float)
    if np.isclose(a, 0.0):
        return x
    z = np.sign(x) * np.minimum(np.abs(x), (np.abs(x) / a)**0.2)
    for _ in range(30):
        step = (z + a * z**5 - x) / (1.0 + 5.0 * a * z**4)
        z_new = z - step
        if np.max(np.abs(z_new - z)) < 1e-12:
            z = z_new
            break
        z = z_new
    return z
...
def inverse_quad_cubic(x, a):
    x = np.asarray(x, dtype=float)
    if np.isclose(a, 0.0):
        return x
    return (np.cbrt(1.0 + 3.0 * a * x) - 1.0) / a


TRANSFORMS = {
    "inverse_cubic": inverse_cubic,
    "inverse_quintic": inverse_quintic,
    "inverse_quad": inverse_quad_cubic,
}

# Strengths are family-specific; they are NOT intended to be numerically comparable.
STRENGTHS = {
    "inverse_cubic": [0.1302579, 0.22996560, 0.3532381, 0.5265986, 1.3483315],
    "inverse_quintic": [0.0098076, 0.0152675, 0.0204604, 0.0258628, 0.038476],
    "inverse_quad": [0.1659280, 0.2476384, 0.3223790, 0.3994894, 0.5912237],
}
